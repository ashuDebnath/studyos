...
function checkLimit(userId) {
  ...
  if (n >= DAILY_LIMIT)
    throw Object.assign(
      new Error(
        `Daily AI limit of ${DAILY_LIMIT} calls reached. Resets at midnight.`,
      ),
      { statusCode: 429 },
    );
...
${context}`;
  ...
  const cards = await withRetry(async () => {
    ...
    if (!Array.isArray(data) || !data.length)
      throw new Error("Bad flashcard response");
    return data.map((c) => ({
...
${context}`;
  ...
  return await withRetry(async () => {
    ...
    const data = JSON.parse(cleanJSON(r.response.text()));
    if (!Array.isArray(data) || !data.length)
      throw new Error("Bad quiz response");
    return data.map((q) => ({
...
${context}`;
  ...
  const chatHistory = [
    { role: "user", parts: [{ text: systemPrompt }] },
    {
      ...
      parts: [
        {
          text: "Understood. I will only answer based on your notes. What would you like to know?",
        },
      ],
    },
    ...history
